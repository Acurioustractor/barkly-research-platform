-- Drop existing documents table and recreate without auth schema dependency
DROP TABLE IF EXISTS documents CASCADE;

-- Create documents table without auth.users foreign key
CREATE TABLE documents (
    ...
    uploaded_by UUID NOT NULL, -- No foreign key to auth.users for now
...
CREATE INDEX idx_documents_community_status ON documents(community_id, processing_status, created_at DESC);
CREATE INDEX idx_documents_hash_dedup ON documents(sha256_hash, community_id);
CREATE INDEX idx_documents_uploader ON documents(uploaded_by, created_at DESC);

SELECT 'Documents table created successfully with proper foreign keys' as status;