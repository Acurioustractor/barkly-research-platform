  ...
  try {
    ...
    for (let i = 0; i < documents.length; i++) {
      for (let j = i + 1; j < documents.length; j++) {
        ...
        // Find shared themes
        const themes1 = new Set(doc1.themes.map(t => t.theme));
        const themes2 = new Set(doc2.themes.map(t => t.theme));
        const sharedThemes = [...themes1].filter(t => themes2.has(t));
        
        if (sharedThemes.length > 0) {
          // Calculate strength based on number of shared themes and confidence
          const avgConfidence1 = doc1.themes.reduce((sum, t) => sum + t.confidence, 0) / doc1.themes.length;
          const avgConfidence2 = doc2.themes.reduce((sum, t) => sum + t.confidence, 0) / doc2.themes.length;
