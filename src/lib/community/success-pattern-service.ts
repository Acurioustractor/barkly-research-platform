...
import { getSuccessPatternsByCommunity } from './success-pattern-db-service';
...
/**
 * Get success patterns for a community (wrapper for DB service)
 */
export async function getSuccessPatterns(communityId: string): Promise<SuccessPattern[]> {
  try {
    ...
    return patterns.map(p => ({
      ...
      resources: [],
      stakeholders: [],
      scalability: p.scalability as any,
      culturalSafety: p.cultural_safety as any,
