...
/**
 * Get service gaps from database
 */
export async function getServiceGaps(communityId: string): Promise<ServiceGap[]> {
  try {
    ...
    return (data || []).map((item: any) => ({
      ...
      affectedPopulation: item.affected_population || { groups: [], estimatedSize: 0, demographics: [] },
      currentAlternatives: item.current_alternatives || [],
      barriers: item.barriers || [],
      recommendations: item.recommendations || [],
      relatedNeeds: item.related_needs || [],
      culturalConsiderations: item.cultural_considerations || [],
      successExamples: item.success_examples || [],
      confidence: item.confidence || 0,
      geographicScope: item.geographic_scope || 'local',
      trends: item.trends || { worsening: false, stable: true, improving: false, emerging: false }
    }));
  } catch (error) {
    return [];
  }
